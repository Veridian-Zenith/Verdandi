...
public:
    ...
    static constexpr CapabilitySet all() {
        return CapabilitySet{0xFFFFFFFFFFFFFFFFull};
    }

    constexpr bool has(Cap c) const { return bits_ & static_cast<uint64_t>(c); }
    constexpr CapabilitySet& grant(Cap c) { bits_ |= static_cast<uint64_t>(c); return *this; }
    constexpr CapabilitySet& revoke(Cap c) { bits_ &= ~static_cast<uint64_t>(c); return *this; }
...
constexpr bool allows(Op op, const CapabilitySet& s) {
    switch (op) {
    case Op::Spawn:           return s.has(Cap::ProcessSpawn);
    case Op::OpenRead:        return s.has(Cap::FileRead);
    case Op::OpenWrite:       return s.has(Cap::FileWrite);
    case Op::BindListen:      return s.has(Cap::NetBind);
    case Op::OutboundConnect: return s.has(Cap::NetConnect);
    case Op::GenerateCode:    return s.has(Cap::ExecJit);
    case Op::OpenDevice:      return s.has(Cap::DeviceOpen);
    }
    return false;
}
